...
interface RetweetMenuProps {
    ...
    singlePost?:boolean
    retweetMenuRef:React.RefObject<HTMLDivElement | null>
    postState?:PostStateProps
    handleUndoneRetweet:React.MouseEventHandler<HTMLDivElement>
}
const RetweetMenu = ({onclick,singlePost,retweetMenuRef,postState,handleUndoneRetweet}:RetweetMenuProps) => {
    return (
        <div  ref={retweetMenuRef} className ={` z-30 absolute bg-black  shadow-[rgba(255,255,255,0.2)_0px_0px_15px,rgba(255,255,255,0.18)_0px_0px_3px_1px]  p-3 rounded-lg w-max h-max top-2 ${singlePost ? "left-9":"left-20"} flex flex-col gap-5`}>
