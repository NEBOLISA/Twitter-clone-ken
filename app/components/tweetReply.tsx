...
import React, { CSSProperties, HTMLElementType, useEffect, useRef, useState } from 'react'
                        ...
                        <div className='w-full relative'>
                                ...
                                ?.map((f, index) => (
                                    <div key={index} className="w-full h-full">
                                        {f.type.startsWith("image/") ? (
                                            <img
                                                src={replyPreview[index]} // Get the corresponding preview
                                                alt={`Preview ${index}`}
                            ...
                            <div className='flex items-center justify-between relative'>
                                ...
                                <form onSubmit={submitReply} className='w-18 flex justify-end'>
                                    <div className={`rounded-full  items-end w-18    ${replyText.length > 0 ? "bg-white" : "bg-[#787a7a]"}  text-[#000000] ${!isLoading ? "px-4 py-2" : "px-4 py-1"} `}>
                                        <button type="submit" className='font-bold w-18 '>{isLoading
                                            ?
